...
func NewTools(currentOrganization config.Organization, organizations []config.Organization, chaincodes []config.Chaincode, network string, capabilities config.Capabilities) *ToolsNode {
	...
	volumes := []*yaml.Node{
		...
		yaml.ScalarNode(fmt.Sprintf("./%s/data/channels:%s/channels", domain, constants.DEFAULT_FABRIC_DIRECTORY)),

		yaml.ScalarNode(fmt.Sprintf("./%[1]s/data/certificate-authority/organizations:%[2]s/%[1]s", domain, constants.DEFAULT_FABRIC_DIRECTORY)),
	}
	...
	for _, organization := range organizations {
		...
		if len(organization.Orderers) > 0 {
			for _, orderer := range organization.Orderers {
				...
				ordererHostDir := fmt.Sprintf("./%[1]s/data/certificate-authority/organizations/ordererOrganizations/%[1]s/orderers/%[2]s.%[1]s", organization.Domain, orderer.Subdomain)
		...
		peerHostDir := fmt.Sprintf("./%[1]s/data/certificate-authority/organizations/peerOrganizations/%[1]s", organization.Domain)
		...
		for _, peer := range organization.Peers {
			peerHostDir := fmt.Sprintf("./%[1]s/data/certificate-authority/organizations/peerOrganizations/%[1]s/peers/%[2]s.%[1]s", organization.Domain, peer.Subdomain)
