		...
		if organization.Bootstrap {
			for _, profile := range f.config.Profiles {
				...
				args := []string{
					...
					"configtxgen",
					"-outputBlock", fmt.Sprintf("%s/channel/%s.block", DEFAULT_FABRIC_DIRECTORY, strings.ToLower(profile.Name)),
					"-profile", profile.Name,
					"-channelID", strings.ToLower(profile.Name),
					"-configPath", fmt.Sprintf("%s/", DEFAULT_FABRIC_DIRECTORY),
				}
...
func (f *Fabric) FetchGenesisBlock() error {
	...
	caFile := fmt.Sprintf("%s/%s/orderers/%s.%s/tls/ca.crt", DEFAULT_FABRIC_DIRECTORY, ordererDomain, orderer.Hostname, ordererDomain)
