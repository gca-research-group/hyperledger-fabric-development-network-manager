...
func NewPeer(
	...
	node := yaml.MappingNode(
		...
		yaml.ScalarNode(ResolvePeerImage(peer)),
		...
		yaml.ScalarNode("depends_on"),
		yaml.MappingNode(
			yaml.ScalarNode(ResolveCouchDBDomain(peer.Subdomain, domain)),
			yaml.MappingNode(yaml.ScalarNode("condition"), yaml.ScalarNode("service_healthy")),
		),
	)
...
func (pn *PeerNode) ExposePort() *PeerNode {
	...
	pn.GetOrCreateValue("ports", yaml.SequenceNode(yaml.ScalarNode(fmt.Sprintf("%d:%d", pn.peer.ExposePort, ResolvePeerPort(pn.peer.Port)))))
...
func (pn *PeerNode) WithVolumes() *PeerNode {
	...
	pn.GetOrCreateValue("volumes", yaml.SequenceNode(volumes...))
