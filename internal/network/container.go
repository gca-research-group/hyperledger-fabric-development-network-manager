...
func (cm *ContainerManager) RunOrdererContainers() error {
	...
	for _, organization := range cm.config.Organizations {
		for _, orderer := range organization.Orderers {
			...
			if err := compose.RunContainerFromTheDockerComposeFile(cm.executor, cm.network, config); err != nil {
...
func (cm *ContainerManager) RunPeerContainers() error {
	...
	for _, organization := range cm.config.Organizations {
		for _, peer := range organization.Peers {
			...
			if err := compose.RunContainerFromTheDockerComposeFile(cm.executor, cm.network, peerFile); err != nil {
...
func (cm *ContainerManager) StopCertificateAuthorities() error {
	...
	for _, organization := range cm.config.Organizations {
		...
		if err := compose.StopContainerFromTheDockerComposeFile(cm.executor, cm.network, config); err != nil {
